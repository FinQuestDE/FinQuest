...
const FQ = {
  ...
  async register(name, email, password) {
    ...
    try {
      // ── Nutzername auf Eindeutigkeit prüfen ────────────────
      try {
        const db  = window.FirebaseDB.db;
        const q   = window.FirebaseDB.query(
          window.FirebaseDB.collection(db, 'users'),
          window.FirebaseDB.where('username', '==', name)
        );
        const snap = await window.FirebaseDB.getDocs(q);
        if (!snap.empty) {
          return { ok: false, msg: 'Dieser Nutzername ist bereits vergeben. Bitte wähle einen anderen.' };
        }
      } catch(checkErr) {
        // Falls Firestore noch nicht bereit – Prüfung überspringen, nicht abbrechen
        console.warn('Nutzername-Prüfung fehlgeschlagen (nicht kritisch):', checkErr);
      }
      ...
      // Lokalen Speicher vorbereiten (aber NICHT einloggen – erst nach E-Mail-Bestätigung)
      const users = this.getUsers();
      users[email] = {
        id: userId,
      ...
      // KEIN localStorage.setItem('fq_current_user') – User muss erst E-Mail bestätigen

      // Verifikations-E-Mail senden
      try {
        await window.FirebaseAuth.sendEmailVerification(cred.user);
      } catch(verifErr) {
        console.warn('Verifikations-E-Mail Fehler (nicht kritisch):', verifErr);
      }

      // Firebase-Session beenden – User darf erst nach Bestätigung eingeloggt werden
    ...
    try {
      ...
      if (!cred.user.emailVerified) {
        ...
        return {
          ...
          msg: 'Bitte bestätige zuerst deine E-Mail-Adresse. Schaue in deinen Posteingang (auch Spam).',
          email,
        };
      }
  ...
  async sendPasswordReset(email) {
    const isLoaded = await this._ensureFirebase();
    if (!isLoaded) return { ok: false, msg: 'Verbindung zum Server fehlgeschlagen.' };
    try {
      await window.FirebaseAuth.sendPasswordResetEmail(window.FirebaseAuth.auth, email);
      return { ok: true };
    } catch(e) {
      let msg = e.message || 'Ein Fehler ist aufgetreten.';
      if (e.code === 'auth/user-not-found')  msg = 'Kein Konto mit dieser E-Mail gefunden.';
      if (e.code === 'auth/invalid-email')   msg = 'Ungültige E-Mail-Adresse.';
      if (e.code === 'auth/too-many-requests') msg = 'Zu viele Anfragen. Bitte warte kurz.';
      return { ok: false, msg };
    }
  },
  ...
  async resendVerification(email, password) {
    const isLoaded = await this._ensureFirebase();
    if (!isLoaded) return { ok: false, msg: 'Verbindung fehlgeschlagen.' };
    try {
      const cred = await window.FirebaseAuth.signInWithEmailAndPassword(window.FirebaseAuth.auth, email, password);
      if (cred.user.emailVerified) {
        await window.FirebaseAuth.signOut(window.FirebaseAuth.auth);
        return { ok: false, msg: 'E-Mail ist bereits bestätigt. Du kannst dich jetzt einloggen.' };
      }
      await window.FirebaseAuth.sendEmailVerification(cred.user);
      await window.FirebaseAuth.signOut(window.FirebaseAuth.auth);
      return { ok: true };
    } catch(e) {
      let msg = e.message || 'Fehler beim Senden.';
      if (e.code === 'auth/invalid-credential') msg = 'E-Mail oder Passwort falsch.';
      if (e.code === 'auth/too-many-requests')  msg = 'Zu viele Anfragen. Bitte warte.';
      return { ok: false, msg };
    }
  },
...
<div id="authOverlay" class="auth-overlay" onclick="FQ.closeAuthIfOutside(event)">
  <div class="auth-modal">
    ...
    <div id="form-login" class="auth-form">
      ...
      <div class="auth-field">
        <label style="display:flex;justify-content:space-between;align-items:center">
          Passwort
          <button onclick="FQ.showPasswordReset()" style="background:none;border:none;color:#00D48A;font-size:11px;cursor:pointer;font-family:'DM Sans',sans-serif;font-weight:600;padding:0;text-decoration:underline">Passwort vergessen?</button>
        </label>
    ...
    <div id="form-reset" class="auth-form" style="display:none">
      <div class="auth-logo">🔑 <span style="color:#F0F4FF">Fin</span><span style="color:#00D48A">Quest</span></div>
      <p class="auth-subtitle">Gib deine E-Mail ein – wir schicken dir einen Reset-Link.</p>
      <div class="auth-field">
        <label>E-Mail</label>
        <input type="email" id="reset-email" placeholder="du@beispiel.de" autocomplete="email">
      </div>
      <div class="auth-error" id="reset-msg" style="min-height:18px;margin-bottom:8px;text-align:center;font-size:13px"></div>
      <button class="auth-btn" id="reset-btn" onclick="FQ.doPasswordReset()">Reset-Link senden</button>
      <div class="auth-demo">
        <button onclick="FQ.showPasswordReset(false)">← Zurück zum Login</button>
      </div>
    </div>
  ...
  showPasswordReset(show = true) {
    document.getElementById('form-login').style.display    = show ? 'none' : 'block';
    document.getElementById('form-register').style.display = 'none';
    document.getElementById('form-reset').style.display    = show ? 'block' : 'none';
  ...
  async doLogin() {
    ...
    if (res.ok) {
      ...
      err.innerHTML = res.msg + '<br><button onclick="FQ._resendFromModal(\'' + res.email + '\')" style="margin-top:8px;background:none;border:1px solid rgba(0,212,138,0.4);color:#00D48A;border-radius:8px;padding:6px 14px;font-size:12px;cursor:pointer;font-family:\'DM Sans\',sans-serif">🔄 Erneut senden</button>';
    } else {
  ...
  async doRegister() {
    ...
    if (res.ok) {
      if (res.needsVerification) {
        // Zeige Verifikations-Hinweis statt Dashboard-Redirect
        document.getElementById('form-register').innerHTML = `
          <div style="text-align:center;padding:16px 0">
            <div style="font-size:48px;margin-bottom:16px">📧</div>
            <div class="auth-logo">🚀 <span style="color:#F0F4FF">Fin</span><span style="color:#00D48A">Quest</span></div>
            <h3 style="font-size:18px;font-weight:700;margin:12px 0 8px;color:#F0F4FF">E-Mail bestätigen</h3>
            <p style="font-size:13px;color:#8892B0;line-height:1.65;margin-bottom:20px">
              Wir haben eine Bestätigungs-E-Mail an<br>
              <strong style="color:#F0F4FF">${email}</strong><br>
              gesendet. Klicke auf den Link darin, dann kannst du dich einloggen.
            </p>
            <button class="auth-btn" onclick="FQ.switchTab('login')" style="margin-bottom:10px">Zum Login →</button>
            <p style="font-size:12px;color:#8892B0">Keine E-Mail erhalten? Schau auch im Spam-Ordner.</p>
          </div>`;
      } else {
        this.closeAuth();
        this.updateNavbar();
  ...
  // Hilfs-Funktion: Verifikations-Mail direkt aus Modal erneut senden (ohne Passwort)
  _resendFromModal(email) {
    this.showToast('📧 Bestätigungs-E-Mail wird gesendet…', '#FFD166');
    // Wir nutzen sendPasswordResetEmail als Proxy – nein, wir brauchen das Passwort.
    // Stattdessen zeigen wir eine klare Anweisung:
    const err = document.getElementById('login-error');
    if (err) err.innerHTML = '📧 Bitte logge dich mit deinem Passwort ein – wir senden dann erneut eine Bestätigungs-E-Mail.';
  },

  // Passwort-Reset aus dem Modal heraus
  async doPasswordReset() {
    ...
    if (!email) { msgEl.textContent = 'Bitte E-Mail eingeben.'; msgEl.style.color = '#FF6B6B'; return; }
    btn.disabled = true; btn.textContent = 'Wird gesendet…';
    msgEl.textContent = '';
    const res = await this.sendPasswordReset(email);
    btn.disabled = false; btn.textContent = 'Reset-Link senden';
    if (res.ok) {
      msgEl.textContent = '✅ E-Mail gesendet! Schau in deinen Posteingang.';
      msgEl.style.color = '#00D48A';
    } else {
