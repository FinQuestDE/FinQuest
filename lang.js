...
(function () {
  ...
  /* ══════════════════════════════════════════════════════
     SPRACH-BUTTON – jetzt im Einstellungs-Modal (components.js)
     Diese Funktion bleibt leer; der Button wird von fqInjectSettingsModal() gerendert.
  ══════════════════════════════════════════════════════ */
  function injectLangButton() {
    // Der Sprach-Toggle ist jetzt im ⚙️ Einstellungs-Modal integriert.
    // Kein separater floating Button mehr nötig.
  }
...
/* ══════════════════════════════════════════════════════
     AUTOMATISCHER DEEPL-WÄCHTER (MUTATION OBSERVER) - UPGRADE
  ══════════════════════════════════════════════════════ */
  let deepLTimer = null;
  ...
  const dynamicDeepLObserver = new MutationObserver((mutations) => {
    ...
    for (let mutation of mutations) {
      // Schlägt nun Alarm bei:
      // 1. Neuen HTML-Elementen (Popups/Modals)
      // 2. Reinen Textänderungen im selben Element (Quiz-Fragen)
      // 3. Wenn ein Modal durch eine Klasse (z.B. 'open') sichtbar gemacht wird
      if (
        ...
        hasNewContent = true;
        break; // Sobald wir eine Änderung finden, reicht das, um den Timer zu starten
      }
    }

    if (hasNewContent) {
      // Alten Timer löschen, falls in derselben Millisekunde noch mehr Änderungen passieren
      clearTimeout(deepLTimer);
      
      // Timer neu starten
      deepLTimer = setTimeout(() => {
        // 1. Lokales Wörterbuch zuerst für feste Begriffe anwenden
        if (typeof translatePage === 'function') {
          translatePage();
        }
        
        // 2. DeepL für den Rest der neuen Texte aufrufen
        if (typeof deepLPage === 'function') {
          deepLPage(document.body);
        }
      }, 500); // 500ms warten, bis sich die Seite "beruhigt" hat
    }
  });

  // Wächter starten mit erweiterten Rechten, sobald das Skript lädt
  window.addEventListener('DOMContentLoaded', () => {
    if (LANG === 'en') {
      dynamicDeepLObserver.observe(document.body, { 
        childList: true, 
        subtree: true,
        characterData: true, // <--- WICHTIG: Erfasst dynamisch geänderten Quiz-Text
        attributes: true,    // <--- WICHTIG: Erfasst Änderungen wie class="open"
