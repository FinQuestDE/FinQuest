...
function fqInjectSettingsModal() {
  ...
  var html = '' +
    ...
    '  <div style="background:var(--fq-modal-bg);border:1px solid var(--fq-modal-border);border-radius:24px;width:100%;max-width:480px;display:flex;flex-direction:column;max-height:90vh;overflow:hidden;animation:fqModalIn .25s ease">' +
    '    <!-- Header -->' +
    '    <div style="display:flex;align-items:center;justify-content:space-between;padding:24px 28px 18px;border-bottom:1px solid var(--fq-dd-sep);flex-shrink:0">' +
    '      <div style="font-size:20px;font-weight:700;color:var(--fq-modal-color);font-family:\'Clash Display\',\'DM Sans\',sans-serif">⚙️ Einstellungen</div>' +
    '      <button onclick="fqCloseSettings()" style="width:32px;height:32px;border-radius:50%;background:var(--fq-dd-hover);border:none;color:var(--fq-modal-color);font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center" onmouseover="this.style.background=\'var(--fq-section-bg)\'" onmouseout="this.style.background=\'var(--fq-dd-hover)\'">✕</button>' +
    '    </div>' +
    '    <!-- Body -->' +
    ...
    '      <!-- Sprache -->' +
    '      <div class="fq-set-section">' +
    '        <div class="fq-set-stitle">🌐 Sprache / Language</div>' +
    '        <div style="display:flex;align-items:center;justify-content:space-between;gap:12px">' +
    '          <span style="font-size:13px;color:var(--fq-dd-muted);line-height:1.5">Wechsle zwischen Deutsch und Englisch. Die Seite wird neu geladen.</span>' +
    '          <button id="fq-set-lang-btn" class="notranslate" translate="no" onclick="(function(){var l=localStorage.getItem(\'fq_lang\')||\'de\';localStorage.setItem(\'fq_lang\',l===\'de\'?\'en\':\'de\');location.reload();})()" style="flex-shrink:0;display:inline-flex;align-items:center;gap:7px;padding:10px 20px;background:rgba(0,212,138,0.08);border:1.5px solid rgba(0,212,138,0.35);border-radius:10px;color:var(--color-primary);font-family:\'DM Sans\',sans-serif;font-size:13px;font-weight:700;cursor:pointer;transition:all .2s;white-space:nowrap" onmouseover="this.style.background=\'rgba(0,212,138,0.18)\';this.style.borderColor=\'rgba(0,212,138,0.65)\'" onmouseout="this.style.background=\'rgba(0,212,138,0.08)\';this.style.borderColor=\'rgba(0,212,138,0.35)\'">🌐 <span id="fq-set-lang-label" class="notranslate" translate="no"></span></button>' +
    '        </div>' +
    '      </div>' +
    '      <!-- Gefahrenzone -->' +
    ...
    '        <p style="font-size:12px;color:var(--fq-dd-muted);margin-bottom:14px;line-height:1.65">Diese Aktion ist <strong style="color:var(--fq-modal-color)">unwiderruflich</strong>. Alle Daten, XP, Fortschritte und Käufe werden dauerhaft gelöscht.</p>' +
    ...
    '          <div style="font-size:11px;color:var(--fq-dd-muted)">Tippe <strong style="color:var(--fq-modal-color)">LÖSCHEN</strong> zur Bestätigung</div>' +
  ...
  // Sprach-Label dynamisch setzen
  var langLabel = document.getElementById('fq-set-lang-label');
  if (langLabel) {
    var curLang = localStorage.getItem('fq_lang') || 'de';
    langLabel.textContent = curLang === 'de' ? 'Switch to English' : 'Zu Deutsch wechseln';
  }
}
...
function injectGlobalComponentStyles() {
  ...
  style.textContent = [
    /* ── Design-Token-Variablen (game = default, werden von theme.js für clean überschrieben) ── */
    ':root{' +
      '--color-primary:#00D48A;--color-secondary:#1A1A2E;--color-accent:#FFD166;--color-highlight:#FF6B6B;' +
      '--color-surface:#16213E;--color-surface-2:#0F3460;--color-text-main:#F0F4FF;--color-text-muted:#8892B0;' +
      '--color-border:rgba(0,212,138,0.2);--color-footer:#0D0D1A;' +
      '--p:#00D48A;--s:#1A1A2E;--acc:#FFD166;--hl:#FF6B6B;--su:#16213E;--su2:#0F3460;--tx:#F0F4FF;--mu:#8892B0;--bo:rgba(0,212,138,0.2);' +
      /* component-spezifische Tokens */
      '--fq-nav-bg:rgba(26,26,46,0.92);--fq-nav-border:rgba(0,212,138,0.2);' +
      '--fq-pill-bg:#16213E;--fq-pill-border:rgba(0,212,138,0.25);--fq-pill-color:#F0F4FF;' +
      '--fq-dd-bg:#16213E;--fq-dd-border:rgba(255,255,255,0.1);--fq-dd-color:#F0F4FF;--fq-dd-muted:#8892B0;' +
      '--fq-dd-sep:rgba(255,255,255,0.07);--fq-dd-hover:rgba(255,255,255,0.07);' +
      '--fq-modal-bg:#16213E;--fq-modal-border:rgba(255,255,255,0.1);--fq-modal-color:#F0F4FF;' +
      '--fq-section-bg:#0F3460;--fq-section-border:rgba(255,255,255,0.07);' +
      '--fq-input-bg:#16213E;--fq-input-border:rgba(255,255,255,0.09);--fq-input-color:#F0F4FF;' +
      '--fq-mobile-bg:#16213E;--fq-mobile-border:rgba(0,212,138,0.2);--fq-mobile-color:#F0F4FF;' +
      '--fq-footer-bg:#0D0D1A;--fq-footer-color:#8892B0;--fq-footer-heading:#F0F4FF;' +
    '}',
    /* ── Navbar ── */
    '.fq-navbar{position:sticky;top:0;z-index:999;background:var(--fq-nav-bg);backdrop-filter:blur(14px);border-bottom:1px solid var(--fq-nav-border)}',
    ...
    '.fq-logo-fin{color:var(--fq-dd-color)}.fq-logo-quest{color:var(--color-primary)}',
    '.fq-nav-links{display:flex;align-items:center;gap:28px;list-style:none;margin:0;padding:0}',
    '.fq-nav-links a{color:var(--fq-dd-muted);text-decoration:none;font-size:14px;font-weight:500;transition:color .2s}',
    '.fq-nav-links a:hover{color:var(--fq-dd-color)}',
    ...
    '.fq-btn-primary{background:var(--color-primary);color:#0a0a14}.fq-btn-primary:hover{box-shadow:0 8px 24px rgba(0,212,138,.4)}',
    '.fq-btn-outline{background:transparent;color:var(--fq-pill-color);border:1.5px solid var(--fq-pill-border)}.fq-btn-outline:hover{background:var(--fq-dd-hover)}',
    '.fq-btn-ghost{background:var(--fq-dd-hover);color:var(--fq-pill-color);border:1.5px solid var(--fq-dd-sep)}.fq-btn-ghost:hover{border-color:var(--fq-pill-border)}',
    /* Profil-Pill */
    '.fq-nav-user{display:flex;align-items:center;gap:8px;background:var(--fq-pill-bg);border:1px solid var(--fq-pill-border);border-radius:50px;padding:5px 12px 5px 6px;font-size:13px;text-decoration:none;transition:border-color .2s;color:var(--fq-pill-color);cursor:pointer;user-select:none}',
    '.fq-nav-user:hover{border-color:var(--color-primary)}',
    ...
    '.fq-nav-xp{font-family:\'JetBrains Mono\',monospace;color:var(--color-accent);font-weight:600}',
    /* Hamburger */
    '.fq-hamburger{display:none;flex-direction:column;gap:5px;cursor:pointer;background:none;border:none;padding:4px}',
    '.fq-hamburger span{display:block;width:22px;height:2px;background:var(--fq-mobile-color);border-radius:2px}',
    /* Mobile */
    '.fq-mobile-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.65);z-index:1000}',
    '.fq-mobile-menu{position:fixed;top:0;right:-100%;width:280px;height:100vh;background:var(--fq-mobile-bg);z-index:1001;padding:72px 28px 40px;display:flex;flex-direction:column;gap:4px;border-left:1px solid var(--fq-mobile-border);transition:right .35s cubic-bezier(.25,.46,.45,.94)}',
    '.fq-mobile-menu.open{right:0}',
    '.fq-mobile-menu a{color:var(--fq-mobile-color);text-decoration:none;font-size:17px;font-weight:600;padding:11px 0;border-bottom:1px solid var(--fq-dd-sep);display:block}',
    '.fq-mobile-close{position:absolute;top:18px;right:18px;background:none;border:none;color:var(--fq-mobile-color);font-size:22px;cursor:pointer}',
    /* Footer */
    '.fq-footer{background:var(--fq-footer-bg);border-top:1px solid var(--fq-dd-sep);padding:56px 24px 0;margin-top:40px}',
    ...
    '.fq-footer-brand p{font-size:14px;color:var(--fq-footer-color);line-height:1.65;max-width:260px;margin-top:12px}',
    '.fq-footer-socials{display:flex;gap:10px;margin-top:20px}',
    '.fq-footer-soc{width:36px;height:36px;border-radius:8px;background:var(--fq-pill-bg);border:1px solid var(--fq-dd-sep);display:flex;align-items:center;justify-content:center;font-size:15px;text-decoration:none;transition:border-color .2s,transform .2s}',
    '.fq-footer-soc:hover{border-color:var(--fq-nav-border);transform:translateY(-2px)}',
    '.fq-footer-col h4{font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;margin-bottom:14px;color:var(--fq-footer-heading)}',
    '.fq-footer-col ul{list-style:none;display:flex;flex-direction:column;gap:9px;padding:0;margin:0}',
    '.fq-footer-col ul a{font-size:13px;color:var(--fq-footer-color);text-decoration:none;transition:color .2s}',
    '.fq-footer-col ul a:hover{color:var(--fq-footer-heading)}',
    '.fq-footer-bottom{border-top:1px solid var(--fq-dd-sep);padding:18px 0;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;font-size:12px;color:rgba(255,255,255,.25)}',
    '.fq-footer-bottom a{color:inherit;text-decoration:none}',
    /* Settings Modal & Sections */
    '.fq-set-section{background:var(--fq-section-bg);border:1px solid var(--fq-section-border);border-radius:16px;padding:18px 20px}',
    '.fq-set-stitle{font-size:13px;font-weight:700;color:var(--fq-modal-color);margin-bottom:12px;font-family:"Clash Display","DM Sans",sans-serif}',
    '.fq-set-input{width:100%;background:var(--fq-input-bg);border:1.5px solid var(--fq-input-border);border-radius:12px;padding:11px 16px;font-family:"DM Sans",sans-serif;font-size:14px;color:var(--fq-input-color);outline:none;transition:border-color .2s,box-shadow .2s}',
    '.fq-set-input:focus{border-color:var(--color-primary);box-shadow:0 0 0 3px rgba(0,212,138,0.12)}',
    '.fq-set-input::placeholder{color:var(--fq-dd-muted)}',
    '.fq-set-btn{display:inline-flex;align-items:center;justify-content:center;padding:10px 18px;background:var(--color-primary);color:#0a0a14;border:none;border-radius:10px;font-weight:700;font-size:13px;cursor:pointer;transition:all .2s;font-family:"DM Sans",sans-serif;white-space:nowrap}',
    ...
    '.fq-dd-btn{display:flex;align-items:center;gap:10px;padding:10px 12px;border-radius:10px;cursor:pointer;font-size:13px;font-weight:600;color:var(--fq-dd-color);background:transparent;border:none;width:100%;font-family:"DM Sans",sans-serif;text-align:left;transition:background .15s}',
    '.fq-dd-btn:hover{background:var(--fq-dd-hover)}',
...
function fqUpdateNavbar() {
  try {
    ...
    if (actionsEl) {
      var pillSt  = 'display:inline-flex;align-items:center;gap:8px;background:var(--fq-pill-bg);border:1px solid var(--fq-pill-border);border-radius:50px;padding:5px 14px 5px 6px;font-size:13px;font-weight:500;text-decoration:none;color:var(--fq-pill-color);cursor:pointer;font-family:DM Sans,sans-serif;line-height:1';
      ...
      var xpSt    = 'font-family:JetBrains Mono,monospace;color:var(--color-accent);font-weight:600;font-size:13px';
      var ddSt    = 'display:none;position:absolute;top:calc(100% + 10px);right:0;background:var(--fq-dd-bg);border:1px solid var(--fq-dd-border);border-radius:16px;padding:8px;min-width:215px;box-shadow:0 16px 40px rgba(0,0,0,0.55);z-index:9999';
      var btnSt   = 'display:flex;align-items:center;gap:10px;padding:10px 12px;border-radius:10px;cursor:pointer;font-size:13px;font-weight:600;color:var(--fq-dd-color);background:transparent;border:none;width:100%;font-family:DM Sans,sans-serif;text-align:left;';
      ...
      actionsEl.innerHTML =
        '<div style="position:relative;z-index:1000" id="fq-profile-wrap">' +
          '<a href="dashboard.html" id="fq-profile-pill" style="' + pillSt + '">' +
            ...
            '<span style="font-size:13px;font-weight:600;color:var(--fq-pill-color)">' + firstName + '</span>' +
            '<span style="' + xpSt + '">&#9889;' + xp + '</span>' +
            '<span style="font-size:10px;color:var(--fq-dd-muted);margin-left:2px">&#9660;</span>' +
          '</a>' +
          '<div id="fq-nav-dd" style="' + ddSt + '">' +
            '<div style="padding:10px 12px 12px;border-bottom:1px solid var(--fq-dd-sep);margin-bottom:6px">' +
              '<div style="font-size:14px;font-weight:700;color:var(--fq-dd-color)">' + user.name + '</div>' +
              '<div style="font-size:11px;color:var(--fq-dd-muted);margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:185px">' + email + '</div>' +
            '</div>' +
            '<button onclick="fqHandleFriends()" style="' + btnSt + '" onmouseover="this.style.background=\'var(--fq-dd-hover)\'" onmouseout="this.style.background=\'transparent\'">&#128101; Freunde <span id="fq-req-badge" style="display:none;margin-left:auto;background:#FF6B6B;color:#fff;font-size:9px;font-weight:700;padding:1px 7px;border-radius:50px">0</span></button>' +
            '<button onclick="fqOpenSettings()" style="' + btnSt + '" onmouseover="this.style.background=\'var(--fq-dd-hover)\'" onmouseout="this.style.background=\'transparent\'">&#9881; Einstellungen</button>' +
            '<div style="height:1px;background:var(--fq-dd-sep);margin:4px 0"></div>' +
    ...
    if (mobileAuth) {
      mobileAuth.innerHTML =
        '<a href="dashboard.html" onclick="fqCloseMenu&&fqCloseMenu()" style="color:var(--color-primary)">👤 Mein Dashboard</a>' +
        '<a href="#" onclick="fqOpenSettings&&fqOpenSettings()" style="color:var(--fq-mobile-color)">⚙️ Einstellungen</a>' +
        '<a href="#" onclick="localStorage.removeItem(\'fq_current_user\');window.location.href=\'finquest.html\'" style="color:var(--color-highlight)">🚪 Ausloggen</a>';
    }
  } catch(e) {}
}
